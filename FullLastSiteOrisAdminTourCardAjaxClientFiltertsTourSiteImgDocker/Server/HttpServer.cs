...
{
    private readonly HttpListener _listener = new(); //прослушка и главное сервера
    private readonly string _staticFolder; //статистика
    private readonly string _connString; //подключение к бд
    private readonly PageRenderer _pageRenderer; //шаблонизатор
    private readonly AdminPanel _adminPanel; //админ панель
    private readonly Dictionary<string, DateTime> _sessions = new(); //сессия админки
    ...
    private async void HandleRequest(HttpListenerContext ctx)
    {
        ...
        try
        {
            ...
            {
                bool isAuth = IsAuthenticated(req); //авторизиация пользователя
                ...
                {
                    if (req.HttpMethod == "POST")
                        await _adminPanel.HandleCrud(ctx); //дозволенность crud операций
                    else
                        await _adminPanel.ServeDashboard(ctx); //прорисовка туров на странице
                    return;
                }

                var editMatch = Regex.Match(path, @"^/admin/edit/(\d+)$"); //редактирование туров
            ...
            if (path == "/api/tours") //ajax endpoints
            {
                ...
                string json = $"{{\"cards\":\"{HttpUtility.JavaScriptStringEncode(cardsHtml)}\",\"total\":{total}}}"; //ответ на запрос о фильтрах
                byte[] buffer = Encoding.UTF8.GetBytes(json);

                resp.ContentType = "application/json; charset=utf-8";
                resp.ContentLength64 = buffer.Length;
                await resp.OutputStream.WriteAsync(buffer); //отправка ответа
                return;
            }

            var tourMatch = Regex.Match(path, @"^/tour/(\d+)$", RegexOptions.IgnoreCase); //роутинг страницы
            ...
            if (path == "/" || path == "/index.html")  //создание главной страницы
            {
    ...
    private bool IsAuthenticated(HttpListenerRequest req) //буль авторизации
    {
        var sid = req.Cookies["session"]?.Value;
        return sid != null && _sessions.TryGetValue(sid, out var exp) && exp > DateTime.UtcNow;
    }

    private void CreateSession(HttpListenerResponse resp) //подставка cookie под авторизацию и замер времени
    {
    ...
    private void Logout(HttpListenerContext ctx) //выход из админки
    {
    ...
    private async Task HandleLogin(HttpListenerContext ctx)  //проверка логина и пароля
    {
    ...
    private async Task ServeLoginPage(HttpListenerResponse resp) //страница логина
    {
        const string html = @"
...
<form method='post'>
    ...
    private async Task ServeFile(HttpListenerResponse resp, string filePath) //загрузка статистических файлов
    {
